...
@router.post(
    "/",
    response_description="Add a new Quiz",
    status_code=status.HTTP_201_CREATED,
    response_model=QuizOut,
)
async def create_quiz(
    req: Request, create_quiz: CreateQuiz, current_admin=Depends(get_current_admin)
):
...
@router.get(
    "/search",
    description="Search Quiz By Category or Quiz Name",
...
@router.patch(
    "/activate", description="Make Quiz Active", status_code=status.HTTP_200_OK
)
async def activate_quiz(
    req: Request, id: str, current_admin=Depends(get_current_admin)
):
    return service_active_deactive_quiz(id, current_admin, True)


@router.patch(
    "/deactivate", description="Make Quiz Deactive", status_code=status.HTTP_200_OK
)
async def deactivate_quiz(
    req: Request, id: str, current_admin=Depends(get_current_admin)
):
